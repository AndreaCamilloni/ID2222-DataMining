...
public class Jabeja {
  ...
  private void sampleAndSwap(int nodeId) {
    ...
    if (partner != null){
      ...
      numberOfSwaps++; //add number of swap
    }
  }
  ...
  public Node findPartner(int nodeId, Integer[] nodes){
    ...
    for(int node: nodes){
      ...
      double new_var = Math.pow(dpq, config.getAlpha()) + Math.pow(dqp, config.getAlpha());

      // Here is for the first task
      // if(new_var * T > old && new_var > highestBenefit){
      //   bestPartner = nodeq;
      //   highestBenefit = new_var;
      // }

      // added for the second task
      Random random = new Random();
      double ap = Math.exp(-(1/new_var - 1/old)/T); //bonus part: change this

      // Implementation according to link
      //if (new_var > old && ap > random.nextDouble()){ //move to the new solution
      //  bestPartner = nodeq;
      //  highestBenefit = new_var;
      //}
      //else if(new_var < old){
      ...
      // Other way to implement the second task 
      if(new_var!=old && ap > random.nextDouble() && ap > highestBenefit){ //move to the new solution
        bestPartner = nodeq;
        highestBenefit = new_var;
      }
